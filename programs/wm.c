...
static inline int32_t wm_min_i32(int32_t a, int32_t b) {
    return a < b ? a : b;
}
...
static int wm_layout_alloc_node(wm_state_t* st, uint32_t ws) {
    if (!st) return -1;
    for (int i = 0; i < WM_MAX_LAYOUT_NODES; i++) {
...
static int wm_layout_find_any_leaf(const wm_state_t* st, uint32_t ws) {
    if (!st) return -1;
    if (ws >= WM_MAX_WORKSPACES) return -1;
    for (int i = 0; i < WM_MAX_LAYOUT_NODES; i++) {
        if (!st->layout_nodes[i].used) continue;
...
static void wm_layout_remove_view(wm_state_t* st, uint32_t ws, int view_idx) {
    if (!st) return;
...
static void wm_layout_insert_split(wm_state_t* st, uint32_t ws, int old_view_idx, int new_view_idx) {
    if (!st) return;
    if (ws >= WM_MAX_WORKSPACES) return;
...
static void wm_apply_layout(comp_conn_t* c, wm_state_t* st) {
    ...
    if (root >= 0) {
        typedef struct { int32_t x, y, w, h; } wm_rect_i32_t;
