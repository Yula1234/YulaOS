#ifndef DRIVERS_USB_UHCI_UHCI_HW_H
#define DRIVERS_USB_UHCI_UHCI_HW_H
...
#define UHCI_USBSTS_ERRORINT (1u << 1)
#define UHCI_USBSTS_HSE      (1u << 2)
#define UHCI_USBSTS_HCPE     (1u << 3)
#define UHCI_USBSTS_HCH      (1u << 5)
...
static inline uint32_t uhci_td_maxlen_field(uint16_t len) {
    if (len == 0) {
        return UHCI_TD_TOKEN_MAXLEN_MASK;
    }
